...
class HorizonPlanner:
    ...
    def planHorizon(self, s_init, s_final, waypoint_horizon_indices = None, q_flip_waypoints = None, xyz_ref = None, q_flip_ref = None):
        """
        ...
        for k in range(len(waypoint_horizon_indices)):
            cost += 1*(1 - fabs(dot(S[6:10, waypoint_horizon_indices[k]], q_flip_waypoints[:, k])));
        ...
        self.opti.subject_to(S[0, -1] == s_final[0]); # x coord the same
